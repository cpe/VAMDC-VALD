    ...
    for ib in in_brackets:
        try:
            if before_brackets[i][-1] == '^':
                # remove last string and attach it to the string within
                # brackets
                before_brackets[i] = before_brackets[i][:-1] + "<sup>%s</sup>"\
                        % ib[1:-1]

            elif before_brackets[i][-1] == '_':
                # remove last string and attach it to the string within
                # brackets
                before_brackets[i] = before_brackets[i][:-1]+"<sub>%s</sub>" \
                        % ib[1:-1]
